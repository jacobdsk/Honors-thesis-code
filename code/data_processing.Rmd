...
```{r}
set.seed(123)
file_prep <- function(states, ratio) {
  years = seq(07, 22)
...
create_df <- function(states){
  ...
  fulldf <- map_df(csv_files, ~ read_csv(.x, col_names = c("STATE", "AGEP", "FER", "SEX", "YEAR", "PAOC", "PINCP"), col_types = column_spec))
...
#Manipulating this function will allow for different variables to be scraped
#Refer to PUMS data dictionary for information on what variables are available
remove_cols <- function(df) {
  
  df_edit <- df %>%
    select(
      STATE, AGEP, FER, SEX, YEAR, PAOC, PINCP
      #OC, HINS1, HINS2, DOUT,SCHL, MAR SEMP, YOEP
      )
...
state_list = c("wa", "co", "or")
ratio_v = 1
file_prep(state_list, ratio_v)
